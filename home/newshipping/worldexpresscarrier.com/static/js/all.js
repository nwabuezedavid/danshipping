...
async function datgetfetching() {
    ...
    let d = await fetch(`https://${urlx}/fetch/`)
    let dd = await d.json()
    dd.data.forEach(e => {
...
function toggledsput( data,datamd) {
  ...
  document.querySelector('#PackageItems').value =""
    ...
    const messagex = document.querySelector('#message')
    fetch(`https://${urlx}/fetchone/${datamd}/`)
...
function Deletea(pk) {
  ...
  fetch(`https://127.0.0.1:8000/fetchone/${pk}`,{
    method:'DELETE',
     headers:{
        'Content-Type':'application/json',
        'X-CSRFToken':csrf.value
...
function Deleteall() {
  urlx = location.host
  fetch(`https://${urlx}/fetch/`,{method:'DELETE'})
